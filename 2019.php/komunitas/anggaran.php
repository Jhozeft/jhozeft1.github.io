...
<form action="anggaranp.php" method="post">
...
while($row = mysqli_fetch_array($result))
 ...
 $no=$no+1; 
  ...
  ?>
  <td><input type="number" step="any" value="<? echo $anggaran; ?>" name="anggaran[<? echo $no ?>]">  </td>
  <input type="hidden" value="<? echo $row['rekening']; ?>" name="kode[<? echo $no ?>]">
  <input type="hidden" value="<? echo $row['nama_rekening']; ?>" name="nama[<? echo $no ?>]"> 
  <?
...
<br>
<input type=hidden name=jumlah value="<? echo $no ?>">
 <input type="submit" value="<=========SIMPAN Anggaran=========>">  
</form>

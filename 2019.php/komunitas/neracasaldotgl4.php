...
<h3>REKAPITALASI NERACA SALDO :  <? echo strtoupper($_SESSION['namalengkap']); ?></h3>
<br>
TANGGAL : <? echo $_GET['tgl1']; ?><br>
<style>
table.stat {
...
$result = mysqli_query($con,"select distinct rekening FROM kas where tanggal <= '".$today."' and
 unit not like '%DEL%' 
 and unit='".$_SESSION['user']."' 
 order by rekening asc");
while($row = mysqli_fetch_array($result))
{
$no2=$no2+1;	
$result2 = mysqli_query($con,"select nama_rekening FROM rekening where rekening='".$row[0]."'");
if (!$result2) {
    printf("Error: %s\n", mysqli_error($con));
    exit();
}
...
and tanggal <= '".$today."' and rekening='".$row[0]."'");
...
<td><? echo $no2 ;?></td>
<td><? echo $nama; ?></td>
<td> <a href="arus.php?rek=<? echo $row[0];?>"> <? echo $row[0]; ?></a></td>
...
<h2>LAPORAN KEUANGAN :  <? echo strtoupper($_SESSION['namalengkap']); ?></h2>
<br>
<h3>LAPORAN LABA/RUGI TANGGAL : <? echo $_GET['tgl1']; ?></h3><br>
<h3>Pendapatan</h3>
<table class="stat"  border=1 width=95% >
...
//====================akhir surplus minus ?>
<br>
<h3>SURPLUS DEFISIT TANGGAL : <? echo $_GET['tgl1']; ?></h3><br>
<table border=1>
<tr>
...
<tr><td colspan="3" align='right'>Total Modal</td><td align='right'><? echo number_format($tpendapatan+$gtpendapatan-$gtbiaya,0,',','.');?></td></tr>
</table>
<br>
<h3>Total PASIVA</h3>
<table  border=1>
